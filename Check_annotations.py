...
def generer_data(course_url):
    ...
    lignes = swimmerId + 1
    course_json = read_json_dataroom(course_url+course_name + '.json')
    decoupe_video('./Tracking_Data/from_above/'+course_name +'_from_above.mp4',course_json,course_Espadon,lignes,nbl=8,course_name = course_name,destination = './Tracking_Data/Lignes/')
    ...
    for swimId in swimmerId:#interpolation
        ...
        ligne = swimId +1
        np.save(os.path.join('./Tracking_Data/Positions/', course_name + f"ligne_{ligne}_pos.npy"),interpole)
        



def decoupe_video(file_in,course_json,course_Espadon,lignes,nbl=8,course_name = "",destination = './Tracking_Data/Lignes/',only_annoted = True):# nbl: number of lines
    ...
    for ligne in lignes:
        if ligne >= 1 and ligne <=8:
            ...
            if only_annoted:
                ...
                stop_time = timestr2float(course_json['temps'][f"temps{ligne}"])
            ...
            .output(os.path.join(destination, course_name + f"ligne_{ligne}.m4v"))
    ...
    for course_url in courses_annotees:
        ...
        for i in range(8):
            ...
            if i in swimmerId:
                ...
                try:
                    temps_fin = timestr2float(course_json['temps'][f"temps{swimId+1}"])
